...
export default function ManagerTeachers() {
  ...
  const columns = [
    ...
    { key: "students", label: "Students", align: "center", render: (r) => <span className="text-white/70">{r.students}</span> },
    { key: "groups", label: "Groups", align: "center", render: (r) => <span className="text-white/70">{r.groups}</span> },
    { key: "avgBand", label: "Avg Band", align: "center", render: (r) => <Badge color="secondary">{r.avgBand}</Badge> },
    { key: "rating", label: "Rating", align: "right", render: (r) => (
  ...
  return (
    <div className="space-y-7">
      <SectionHeading title="Teachers" subtitle="Manage your teaching staff and performance."
        action={<button className="btn-primary text-sm"><Plus className="h-4 w-4" /> Add Teacher</button>} />

      <div className="grid grid-cols-2 gap-4 lg:grid-cols-4">
        {[
          { icon: GraduationCap, label: "Teachers", value: teacherPerformance.length, accent: "primary" },
          { icon: Users, label: "Students Taught", value: totalStudents, accent: "secondary" },
          { icon: Star, label: "Avg Rating", value: avgRating, accent: "primary" },
          { icon: Award, label: "Top Band", value: "7.1", accent: "secondary" },
        ].map((s, i) => (
        ...
        {teacherPerformance.map((t, i) => (
          <motion.div key={t.id} initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }}
            ...
            <div className="mt-4 grid grid-cols-3 gap-2 border-t border-white/[0.06] pt-4 text-center">
              <div><div className="text-sm font-semibold text-white">{t.students}</div><div className="text-[10px] text-white/40">Students</div></div>
              <div><div className="text-sm font-semibold text-white">{t.groups}</div><div className="text-[10px] text-white/40">Groups</div></div>
              <div><div className="text-sm font-semibold text-secondary-400">{t.avgBand}</div><div className="text-[10px] text-white/40">Band</div></div>
            </div>
          </motion.div>
        ))}
      </div>

      <Panel title="All Teachers" delay={0.25}>
