...
export default function ManagerGroups() {
  ...
  return (
    <div className="space-y-7">
      <SectionHeading title="Groups" subtitle="All learning groups across the academy."
      ...
      <div className="grid grid-cols-3 gap-4">
        <Panel className="!p-4" delay={0}>
          <div className="flex items-center gap-3">
            ...
            <div><div className="text-xl font-semibold text-white">{academyGroups.length}</div><div className="text-xs text-white/40">Active Groups</div></div>
          </div>
        </Panel>
        <Panel className="!p-4" delay={0.05}>
          <div className="flex items-center gap-3">
            <IconTile icon={Users} accent="secondary" />
            <div><div className="text-xl font-semibold text-white">{totalStudents}</div><div className="text-xs text-white/40">Enrolled</div></div>
          </div>
        </Panel>
          ...
          <motion.div key={g.id} initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }}
            ...
            <div className="mt-4 flex items-center gap-2 rounded-xl bg-white/[0.03] p-2.5">
              ...
              <div className="min-w-0">
                ...
                <div className="text-[10px] text-white/40">Lead teacher</div>
              </div>
            </div>
            ...
            <div className="mt-4 space-y-2.5 text-sm">
              ...
              {g.avgBand && (
                <div>
                  <div className="mb-1 flex justify-between text-white/50"><span>Avg Band</span><span className="text-white/80">{g.avgBand}</span></div>
