...
export default function ManagerPayments() {
  ...
  const columns = [
    ...
    { key: "course", label: "Course", render: (r) => <Badge color="secondary">{r.course}</Badge> },
    { key: "amount", label: "Amount", align: "center", render: (r) => <span className="font-semibold text-white">${r.amount}</span> },
    { key: "date", label: "Date", render: (r) => <span className="text-white/50">{r.date}</span> },
    { key: "status", label: "Status", align: "right", render: (r) => {
      const m = statusMeta[r.status];
      return <Badge color={m.color}><m.icon className="h-3 w-3" /> {m.label}</Badge>;
    }},
  ];

  return (
    <div className="space-y-7">
      <SectionHeading title="Payments" subtitle="Track tuition and revenue."
        action={<button className="btn-ghost text-sm"><Download className="h-4 w-4" /> Export</button>} />

      <div className="grid grid-cols-1 gap-4 sm:grid-cols-3">
        <Panel className="!p-5" delay={0}>
          <div className="flex items-center justify-between">
            <div><div className="text-2xl font-semibold text-emerald-400">${total}</div><div className="text-xs text-white/40">Collected</div></div>
            <IconTile icon={CheckCircle2} accent="green" size="lg" />
          </div>
        </Panel>
        <Panel className="!p-5" delay={0.05}>
          <div className="flex items-center justify-between">
            <div><div className="text-2xl font-semibold text-amber-400">${pending}</div><div className="text-xs text-white/40">Pending</div></div>
            <IconTile icon={Clock} accent="amber" size="lg" />
          </div>
        </Panel>
        <Panel className="!p-5" delay={0.1}>
          <div className="flex items-center justify-between">
            <div><div className="text-2xl font-semibold text-rose-400">${overdue}</div><div className="text-xs text-white/40">Overdue</div></div>
