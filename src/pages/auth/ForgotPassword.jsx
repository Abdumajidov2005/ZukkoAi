...
export default function ForgotPassword() {
  ...
  return (
    <AuthLayout
      ...
      subtitle={sent ? "" : "Enter your email and we'll send you a reset link."}
      footer={<Link to="/login" className="inline-flex items-center gap-1.5 text-primary-400 hover:text-primary-300"><ArrowLeft className="h-3.5 w-3.5" /> Back to login</Link>}
    >
      {sent ? (
        <motion.div initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} className="text-center">
          ...
          <p className="mt-2 text-sm text-white/40">Didn't get it? Check spam or try again in 60s.</p>
        </motion.div>
      ) : (
