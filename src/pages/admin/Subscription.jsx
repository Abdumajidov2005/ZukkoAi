...
export default function AdminSubscription() {
  ...
  return (
    <div className="space-y-7">
      <SectionHeading title="Subscription" subtitle="Manage plans and billing." />

      <div className="grid grid-cols-2 gap-4 lg:grid-cols-4">
        {[
          { icon: Users, label: "Total Subscribers", value: "242", accent: "primary" },
          { icon: DollarSign, label: "MRR", value: `$${mrr.toLocaleString()}`, accent: "secondary" },
          { icon: Star, label: "Pro Conversions", value: "35%", accent: "primary" },
          { icon: TrendingUp, label: "Churn Rate", value: "2.1%", accent: "secondary" },
        ].map((s, i) => (
      ...
      <div className="grid gap-4 lg:grid-cols-3">
        {pricingPlans.map((p, i) => (
          <motion.div key={p.name} initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }}
            ...
            {p.featured && <span className="absolute -top-3 left-1/2 -translate-x-1/2"><Badge color="primary"><Star className="h-3 w-3" /> Most popular</Badge></span>}
      ...
      <Panel title="Subscribers by Plan" delay={0.2}>
