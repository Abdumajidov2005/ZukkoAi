...
export default function StudentVocabulary() {
  ...
  return (
    <div className="space-y-7">
      <SectionHeading
        title={t("pages.student.vocabulary.title")}
        subtitle={t("pages.student.vocabulary.subtitle")}
        action={<Badge color="secondary"><Sparkles className="h-3 w-3" /> {t("pages.student.vocabulary.aiCurated")}</Badge>}
      />
      ...
      <div className="grid gap-4 md:grid-cols-3">
        <Panel className="!p-4 md:col-span-2" delay={0}>
          <div className="flex items-center justify-between text-sm">
            <span className="text-white/70">{t("pages.student.vocabulary.mastery")}</span>
            <span className="font-medium text-white">{learnedCount}/{vocabularyWords.length} {t("pages.student.vocabulary.words")}</span>
          </div>
        ...
        <Panel className="!p-4" delay={0.05}>
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <div className="text-xs text-white/40">{t("pages.student.vocabulary.currentLevel")}</div>
            </div>
          </div>
        </Panel>
      </div>
      ...
      <div className="grid gap-6 lg:grid-cols-2">
        <div>
          <h3 className="mb-3 text-sm font-medium text-white/60">{t("pages.student.vocabulary.flashcards")}</h3>
            ...
            >
              ...
              <div className="glass absolute inset-0 grid place-items-center rounded-3xl p-6 text-center [backface-visibility:hidden]">
                <div>
                  ...
                  <p className="mt-4 text-xs text-white/40">{t("pages.student.vocabulary.tapReveal")}</p>
                </div>
              </div>
              {/* back */}
          ...
          <div className="mt-4 flex items-center justify-between">
            ...
            <div className="flex gap-2">
              <button onClick={flip} className="btn-ghost text-sm"><RotateCw className="h-4 w-4" /> {t("pages.student.vocabulary.flip")}</button>
              <button onClick={markKnown} className="btn-primary text-sm"><Check className="h-4 w-4" /> {t("pages.student.vocabulary.iKnow")}</button>
            </div>
        ...
        <div>
          <div className="mb-3 flex items-center justify-between">
            <h3 className="text-sm font-medium text-white/60">{t("pages.student.vocabulary.wordList")}</h3>
